...
- llama-cpp-2 runs CPU inference with six threads and llguidance JSON constraints. Context is capped at 2,048 tokens, output at 500 tokens, and generation at 90 seconds (180 seconds for structured jobs since 0.1.4). A cached system-prefix state is restored between requests. One worker bounds simultaneous model memory; models unload after five idle minutes.
